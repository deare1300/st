...
int st_netfd_poll(_st_netfd_t *fd, int how, st_utime_t timeout)
{
  ...
  ST_DEBUG("st_netfd_poll timeout:%lld\n", timeout);

  if ((n = st_poll(&pd, 1, timeout)) < 0) {

    ST_DEBUG("st_netfd_poll n:%d, errno:%d\n", n, errno);
    return -1;
  }
  if (n == 0) {
    /* Timed out */
    ...
    ST_DEBUG("st_netfd_poll ETIME n:%d, errno:%d\n", n, errno);
    return -1;
  }
  if (pd.revents & POLLNVAL) {
    errno = EBADF;
    ST_DEBUG("st_netfd_poll EBADF n:%d, errno:%d\n", n, errno);
    return -1;
  }

  ST_DEBUG("st_netfd_poll ok n:%d, errno:%d\n", n, errno);
		       ...
		       st_utime_t timeout)
{
  ...
  ST_DEBUG("line: st_accept init fd->osfd:%d, timeout:%lld\n", fd->osfd, timeout);
  ...
  ST_DEBUG("line: st_accept init client->osfd:%d, timeout:%lld\n", osfd, timeout);
		       ...
		       st_utime_t timeout)
{
  ...
  for ( ; ; ) {
      ...
      ST_DEBUG("st_accept init fd->osfd:%d, timeout:%lld\n", fd->osfd, timeout);
    if (p == NULL) {
    ...
    if (osfd >= 0) {
        ST_DEBUG("st_accept success osfd:%d, timeout:%lld\n", osfd, timeout);
        break;
    }
    if (errno == EINTR)
      continue;
    ...
    /* Wait until the socket becomes readable */
    if (st_netfd_poll(fd, POLLIN, timeout) < 0) {
        ST_DEBUG("st_accept failed fd->osfd:%d, timeout:%lld\n", fd->osfd, timeout);
        return NULL;
    }
  }
...
ssize_t st_read(_st_netfd_t *fd, void *buf, size_t nbyte, st_utime_t timeout)
{
  ...
  while ((n = read(fd->osfd, buf, nbyte)) < 0) {
    ...
    ST_DEBUG("st_read failed will poll fd:%d\n", fd->osfd);
