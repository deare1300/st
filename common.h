...
typedef struct _st_pollq {
  ...
  _st_clist_t fd_links;        /* fd event queue */
} _st_pollq_t;
...
typedef struct _st_eventsys_ops {
  ...
  void (*pollset_add_io_queue) (int fd, _st_clist_t *);
  void (*pollset_del_io_queue) (int fd, _st_clist_t*);
} _st_eventsys_t;
...
typedef struct _st_vp {
  ...
  st_utime_t stat_schedure_time;  /* 缁熻寮�濮嬫椂闂� */
...
#define _ST_FD_POLLQUEUE_PTR(_qp)      \
    ((_st_pollq_t *)((char *)(_qp) - offsetof(_st_pollq_t, fd_links)))
