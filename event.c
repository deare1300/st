...
ST_HIDDEN __thread _st_clist_t epoll_fd_empty;
...
ST_HIDDEN int _st_epoll_init(void)
{
    ...
    epoll_fd_empty.next = &epoll_fd_empty;
    epoll_fd_empty.prev = &epoll_fd_empty;
    ...
    _st_epoll_data->fd_links =
        (_st_clist_t*) calloc(_st_epoll_data->fd_data_size,
                                    sizeof(_st_clist_t));

    for(; c < _st_epoll_data->fd_data_size; ++c)
    {
        _st_epoll_data->fd_links[c].next = &epoll_fd_empty;
        _st_epoll_data->fd_links[c].prev = &epoll_fd_empty;
//        ST_INIT_STATIC_CLIST(&_st_epoll_data->fd_links);
    }

    ST_DEBUG("epoll init size:%d\n", _st_epoll_data->fd_data_size);
...
ST_HIDDEN int _st_epoll_fd_data_expand(int maxfd)
{
    ...
    int c = _st_epoll_data->fd_data_size;
    ...
    p_list = (_st_clist_t *)realloc(_st_epoll_data->fd_links,
                                    n * sizeof(_st_clist_t));
    if (!p_list)
    {
        free(ptr);
        return -1;
    ...
    for(; c < n; ++c)
    {
        p_list[c].next = &epoll_fd_empty;
        p_list[c].prev = &epoll_fd_empty;
    }

    _st_epoll_data->fd_links = p_list;
    _st_epoll_data->fd_data = ptr;
    _st_epoll_data->fd_data_size = n;

    ST_DEBUG("epoll extends size:%d\n", _st_epoll_data->fd_data_size);
    return 0;
}
...
ST_HIDDEN int _st_epoll_pollset_add(struct pollfd *pds, int npds)
{
    ...
    if (i < npds) {
        ...
        ST_DEBUG("_st_epoll_pollset_add errno:%d\n", errno);
        errno = err;
        return -1;
    }
    ...
    if (nfd > 0) {
        ...
        for (q = _ST_IOQ.next; q != &_ST_IOQ; q = q->next) {
            ...
            for (pds = pq->pds; pds < epds; pds++) {
                ...
                fd_link = _st_epoll_data->fd_links + osfd;

                ST_DEBUG("_st_epoll_dispatch old osfd:%d, nfd:%d\n", osfd, nfd);

#if 1
                ST_DEBUG("fd_link:%x, prev:%x, next:%x\n", fd_link, fd_link->prev, fd_link->next);
                for (fd_q = fd_link->next; fd_q != &epoll_fd_empty; fd_q = fd_q->next) {
                    ST_DEBUG("e: %x, prev:%x, next:%x, &epoll_fd_empty:%x\n", fd_q, fd_q->prev,
                        fd_q->next, &epoll_fd_empty);
                }
#endif
        ...
        for (i = 0; i < nfd; i++) {
            ...
            ST_DEBUG("_st_epoll_dispatch new osfd:%d, nfd:%d\n", osfd, nfd);

#if 1
            ST_DEBUG("fd_link:%x, prev:%x, next:%x\n", fd_link, fd_link->prev, fd_link->next);
            for (q = fd_link->next; q != &epoll_fd_empty; q = q->next) {
                ST_DEBUG("e: %x, prev:%x, next:%x, &epoll_fd_empty:%x\n", q, q->prev,
                    q->next, &epoll_fd_empty);
            }
#endif

            for (q = fd_link->next; q != &epoll_fd_empty; q = q->next) {

                pq = _ST_FD_POLLQUEUE_PTR(q);
                ...
                for (pds = pq->pds; pds < epds; pds++) {
                    ...
                    ST_DEBUG("loop _st_epoll_dispatch pds->osfd:%d, npds:%d\n", pds->fd, pq->npds);
                ...
                if (notify) {
                    ST_REMOVE_LINK(&pq->links); // remove io queue
                    ST_REMOVE_LINK(&pq->fd_links);    // remove fd queue
                    pq->on_ioq = 0;
                    /*
                     * Here we will only delete/modify descriptors that
                     * didn't fire (see comments in _st_epoll_pollset_del()).
        ...
        for (i = 0; i < nfd; i++) {
            ...
            ST_DEBUG("_st_epoll_dispatch fired fd:%d, op:%d, del:%d\n", osfd, op, EPOLL_CTL_DEL);
        }
    }
...
ST_HIDDEN void _st_epoll_add_io_queue(int fd, _st_clist_t *link)
{
    _st_clist_t* fd_link = _st_epoll_data->fd_links + fd;
    _st_clist_t* fd_q;

    ST_DEBUG("_st_epoll_add_io_queue osfd:%d\n", fd);

#if 1
    ST_DEBUG("before fd_link:%x, prev:%x, next:%x\n", fd_link, fd_link->prev, fd_link->next);
    for (fd_q = fd_link->next; fd_q != &epoll_fd_empty; fd_q = fd_q->next) {
        ST_DEBUG("e: %x, prev:%x, next:%x, &epoll_fd_empty:%x\n", fd_q, fd_q->prev,
            fd_q->next, &epoll_fd_empty);
    }
#endif
    ST_INSERT_LINK(link, &_st_epoll_data->fd_links[fd]);

#if 1
    ST_DEBUG("after fd_link:%x, prev:%x, next:%x\n", fd_link, fd_link->prev, fd_link->next);
    for (fd_q = fd_link->next; fd_q != &epoll_fd_empty; fd_q = fd_q->next) {
        ST_DEBUG("e: %x, prev:%x, next:%x, &epoll_fd_empty:%x\n", fd_q, fd_q->prev,
            fd_q->next, &epoll_fd_empty);
    }
...
ST_HIDDEN void _st_epoll_del_io_queue(int fd, _st_clist_t *link)
{
    _st_clist_t* fd_link = _st_epoll_data->fd_links + fd;
    _st_clist_t* fd_q;
    ST_DEBUG("_st_epoll_del_io_queue osfd:%d\n", fd);
...
static _st_eventsys_t _st_epoll_eventsys = {
    ...
    _st_epoll_add_io_queue,
    _st_epoll_del_io_queue
};
#endif  /* MD_HAVE_EPOLL */
