...
int st_poll(struct pollfd *pds, int npds, st_utime_t timeout)
{
  ...
  ST_DEBUG("st_poll npds:%d, timeout:%lld\n", npds, timeout);
  ...
  if(_st_eventsys->pollset_add_io_queue)
  {
      for(pd = pds; pd < epd; ++pd)
      {
          (*_st_eventsys->pollset_add_io_queue)(pd->fd, &pq.fd_links);
          ST_DEBUG("st_poll pollset_add_io_queue fd:%d, fd_links:%x, npds:%d\n",
              pd->fd, &pq.fd_links, npds);
      }
  }
  else
  {
      ST_DEBUG("st_poll no pollset_add_io_queue:%d\n", 0);
  }
  pq.pds = pds;
  pq.npds = npds;
  pq.thread = me;
  ...
  if (pq.on_ioq) {
    ...
    _ST_DEL_IOQ(pq);
    if(_st_eventsys->pollset_del_io_queue)
    {
        for(pd = pds; pd < epd; ++pd)
        {
            (*_st_eventsys->pollset_del_io_queue)(pd->fd, &pq.fd_links);
            ST_DEBUG("st_poll pollset_del_io_queue fd:%d, fd_links:%x, npds:%d\n",
                pd->fd, &pq.fd_links, npds);
        }
    }
